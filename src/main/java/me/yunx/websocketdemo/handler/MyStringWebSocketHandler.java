package me.yunx.websocketdemo.handler;
...
//        session.sendMessage(new TextMessage(fakeAi(receiveMessage)));
        ...
        for (WebSocketSession session1 : sessionMap.values()) {
            if (session1.getId().equals(session.getId())) continue;
            session1.sendMessage(new TextMessage(receiveMessage));
        }
    }
